...
static int __write_log_entry(FILE* log_fd)
{
    // always print loop index
...
static void __construct_new_entry()
{
...
int log_manager_add_new()
{
    ...
    __construct_new_entry();
    __write_log_entry(log_fd);
    fflush(log_fd);
    num_entries++;
